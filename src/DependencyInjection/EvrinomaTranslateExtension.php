...
class EvrinomaTranslateExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\TranslateBundle\Entity';
    public const MODEL = 'Evrinoma\TranslateBundle\Model';
    public const ENTITY_FACTORY_TRANSLATE = TranslateFactory::class;
    public const ENTITY_BASE_TRANSLATE = BaseTranslate::class;
    public const DTO_BASE_TRANSLATE = TranslateApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_TRANSLATE !== $config['factory']) {
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, TranslatePass::TRANSLATE_CONSTRAINT)   :
                    $definition->addTag(TranslatePass::TRANSLATE_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $class, string $driver): void
    {
        ...
        $container->addAliases([TranslateCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([TranslateQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaTranslateBundle::BUNDLE;
    }
}
